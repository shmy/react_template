...
const Header: FC<HeaderProps> = props => {
  ...
  const handleLogout = () => {
    http.delete('/logout').then(logout).catch(logout);
  };
  return (
    <Layout.Header className={styles.header}>
      ...
      <Dropdown overlay={
        ...
        <Avatar src={SERVER_STATIC_PATH + (user as any).avatar_url}
                style={{backgroundColor: '#00aaff'}}>{(user as any).real_name || (user as any).username}</Avatar>
      </Dropdown>
    </Layout.Header>
  );
};
